        ...
        <Card className="bg-gradient-to-b from-background to-background/80 border-border/40">
          <CardHeader className="pb-2 px-3 py-2.5">
            <CardTitle className="flex items-center gap-2 text-sm">
              ...
              Members ({selectedSquadData.members.length})
            </CardTitle>
          </CardHeader>
          <CardContent className="p-0">
            <div className="divide-y divide-border/20">
              {selectedSquadData.members.map((member, index) => {
                const memberPayment = selectedSquadData.payments?.find(p => p.user.id === member.id && p.status === 'completed');
                const memberHasPaid = !!memberPayment;
                const isCurrentUser = member.id === user.id;
                
                return (
                  <div key={member.id} className={`px-3 py-2.5 transition-colors ${isCurrentUser ? 'bg-primary/6 border-l-2 border-l-primary/25' : 'hover:bg-muted/20'}`}>
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2.5">
                        ...
                        <div className="flex items-center gap-1.5 min-w-0 flex-1">
                          <div className="min-w-0 flex-1">
                            <p className={`font-medium text-sm ${isCurrentUser ? 'text-primary' : 'text-foreground'} truncate leading-tight`}>
                              {getDisplayName({username: member.user?.username || member.username, displayName: member.user?.displayName, firstName: member.user?.firstName, lastName: member.user?.lastName})}
                              {isCurrentUser && <span className="text-xs ml-1 text-primary/60">(You)</span>}
                            </p>
                            <div className="flex items-center gap-1.5 mt-0.5">
                              {member.role === 'owner' && (
                                <Badge variant="default" className="text-xs h-3.5 bg-amber-500/90 px-1.5 py-0">
                                  Owner
                                </Badge>
                              )}
                              {member.joinedAt && (
