...
const LoginPage = () => {
  ...
  return (
    <div className="min-h-screen relative flex items-center justify-center p-4 overflow-hidden">
      ...
      <div
        className={`absolute top-1/4 left-1/4 w-64 h-64 md:w-96 md:h-96 bg-gradient-to-r from-primary/15 to-blue-500/15 rounded-full blur-3xl transition-all duration-[3000ms] ${
          mounted ? "scale-110 opacity-60" : "scale-100 opacity-40"
        }`}
      />
      <div
        className={`absolute bottom-1/4 right-1/4 w-48 h-48 md:w-80 md:h-80 bg-gradient-to-r from-purple-500/12 to-pink-500/12 rounded-full blur-3xl transition-all duration-[4000ms] delay-500 ${
          mounted ? "scale-125 opacity-50" : "scale-100 opacity-35"
        }`}
      />
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        {[...Array(6)].map((_, i) => (
          <div
            key={i}
            className={`absolute w-2 h-2 bg-gradient-to-r from-primary/30 to-blue-500/30 rounded-full transition-all duration-[${
              6000 + i * 1000
            }ms] ease-in-out ${mounted ? "animate-pulse" : ""}`}
            style={{
              left: `${20 + i * 15}%`,
              top: `${10 + i * 12}%`,
      ...
      <div className="relative w-full max-w-7xl grid lg:grid-cols-5 gap-8 lg:gap-16">
        ...
        <div className="hidden lg:flex lg:col-span-3 flex-col justify-center space-y-12 px-8">
          ...
          <div className="space-y-8">
            <div className="space-y-6">
              <div className="relative">
                <h1
                  className={`text-6xl xl:text-7xl font-black tracking-tight transition-all duration-1000 ${
                    mounted
                ...
                <div
                  className={`absolute -top-4 -right-4 transition-all duration-1500 delay-700 ${
                    mounted
              ...
              >
                <p className="text-xl xl:text-2xl text-slate-600 dark:text-slate-300 leading-relaxed max-w-2xl font-medium">
                  The future of fantasy sports is here. Build winning squads
                  with
          ...
          <div className="grid grid-cols-1 gap-6">
            {features.map((feature, index) => (
              <div
                key={index}
                className={`group relative overflow-hidden p-8 rounded-3xl bg-white/60 dark:bg-slate-800/60 backdrop-blur-xl border border-slate-200/60 dark:border-slate-700/60 hover:bg-white/90 dark:hover:bg-slate-800/90 transition-all duration-500 hover:scale-[1.02] hover:shadow-2xl hover:shadow-primary/10 ${
                  mounted
              ...
              >
                ...
                <div className="relative flex items-center gap-6">
                  <div
                    className={`relative p-4 rounded-2xl bg-gradient-to-r ${feature.gradient} group-hover:scale-110 transition-all duration-500 shadow-lg`}
                  >
                    <feature.icon className="w-7 h-7 text-white drop-shadow-sm" />
                    <div
                      className={`absolute inset-0 bg-gradient-to-r ${feature.gradient} rounded-2xl blur opacity-50 group-hover:opacity-75 transition-opacity duration-500`}
                    />
                  </div>
        ...
        <div className="lg:col-span-2 w-full max-w-md mx-auto lg:max-w-none">
          ...
          <Card
            className={`border-0 shadow-2xl bg-white/98 dark:bg-slate-900/98 backdrop-blur-xl border border-slate-200/50 dark:border-slate-700/50 transition-all duration-1000 ${
