...
import { ArrowLeft, Trophy, Target, Clock, Users, HelpCircle, Zap, Rocket, BarChart3 } from "lucide-react";
...
const howToPlaySections = [
  ...
  },
  {
    ...
    content: [
      ...
      "Rounds 1–4: 30 points per match, 90 per round",
      "Round 5 (Final): Double points — 60 per match, 180 per round",
    ],
  },
  {
    id: "picks",
...
const colorMap: Record<string, { icon: string; bg: string; border: string }> = {
  blue:   { icon: "text-blue-600 dark:text-blue-400",     bg: "bg-blue-100 dark:bg-blue-900/40",     border: "border-blue-200/60 dark:border-blue-800/40" },
  amber:  { icon: "text-amber-600 dark:text-amber-400",   bg: "bg-amber-100 dark:bg-amber-900/40",   border: "border-amber-200/60 dark:border-amber-800/40" },
  purple: { icon: "text-purple-600 dark:text-purple-400", bg: "bg-purple-100 dark:bg-purple-900/40", border: "border-purple-200/60 dark:border-purple-800/40" },
  green:  { icon: "text-green-600 dark:text-green-400",   bg: "bg-green-100 dark:bg-green-900/40",   border: "border-green-200/60 dark:border-green-800/40" },
  orange: { icon: "text-orange-600 dark:text-orange-400", bg: "bg-orange-100 dark:bg-orange-900/40", border: "border-orange-200/60 dark:border-orange-800/40" },
  yellow: { icon: "text-yellow-600 dark:text-yellow-400", bg: "bg-yellow-100 dark:bg-yellow-900/40", border: "border-yellow-200/60 dark:border-yellow-800/40" },
  pink:   { icon: "text-pink-600 dark:text-pink-400",     bg: "bg-pink-100 dark:bg-pink-900/40",     border: "border-pink-200/60 dark:border-pink-800/40" },
};
      ...
      <SheetContent side="bottom" className="h-[85dvh] max-h-[85vh] sm:h-[90dvh] sm:max-h-[90vh] rounded-t-2xl border-0 p-0 z-[10000] flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          <div className="bg-gradient-to-br from-primary/15 via-primary/5 to-transparent px-5 py-5">
            ...
            <p className="text-sm text-muted-foreground mt-1">Predict. Compete. Win your Squad.</p>
          </div>

          <div className="px-4 pb-6">
            {/* How to Play */}
            <h3 className="text-xs font-semibold text-muted-foreground uppercase tracking-widest mt-4 mb-2.5 px-1">How to Play</h3>
            <Accordion type="multiple" className="space-y-2">
              {howToPlaySections.map((section) => {
                const Icon = section.icon;
                const colors = colorMap[section.color];
                return (
                  <AccordionItem key={section.id} value={section.id} className={`border ${colors.border} rounded-xl overflow-hidden`}>
                    <AccordionTrigger className="py-3 px-3 text-sm font-medium hover:no-underline gap-2">
                      <span className="flex items-center gap-2.5">
                        <span className={`w-8 h-8 ${colors.bg} rounded-lg flex items-center justify-center shrink-0`}>
            ...
            <h3 className="text-xs font-semibold text-muted-foreground uppercase tracking-widest mt-5 mb-2.5 px-1">Getting Started</h3>
            <div className="rounded-xl border bg-gradient-to-br from-emerald-50/50 to-teal-50/30 dark:from-emerald-950/20 dark:to-teal-950/10 border-emerald-200/60 dark:border-emerald-800/40 p-4">
              <div className="flex items-center gap-2.5 mb-3">
                ...
                <span className="text-sm font-medium">5 steps to get playing</span>
              </div>
            ...
            {/* FAQ */}
            <h3 className="text-xs font-semibold text-muted-foreground uppercase tracking-widest mt-5 mb-2.5 px-1">Frequently Asked Questions</h3>
            <Accordion type="multiple" className="space-y-2">
              {faqItems.map((item, i) => (
                <AccordionItem key={i} value={`faq-${i}`} className="border border-border/60 rounded-xl overflow-hidden">
